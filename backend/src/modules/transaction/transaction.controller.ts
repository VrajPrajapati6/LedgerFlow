...
import { executeTransfer, getTransactions, getTransactionById, executeDeposit } from "./transaction.service.js";
...
export const depositController = async (
  ...
  try {
    ...
    if (!accountId || !amount) {
      res.status(400).json({ success: false, message: "accountId and amount are required" });
      return;
    }
    const result = await executeDeposit({ accountId, amount: Number(amount), description });
    res.status(201).json({
      success: true,
      data: result,
    });
